...
if [ -n "$PIDS" ]
then
  echo "Docker-release is already running in process $PIDS";
  exit 1
fi
...
LOG_FILE=${LOGGING_PATH}/docker-release.out
ERR_FILE=${LOGGING_PATH}/docker-release.err
...
echo "Giving Docker-release $waitTimeout seconds to start successfully"
...
do
  ...
  then
    echo "Docker-release didn't start within $waitTimeout seconds."
    PIDS=$(pgrep java -lf | grep $APP_NAME | cut -d" " -f1);
    if [ -n "$PIDS" ]
	then
	  echo "Killing $PIDS";
...
if [ $retVal -eq 1 ]
then
  echo "Starting Docker-release failed"
else
  echo "Starting Docker-release succeeded"
fi
